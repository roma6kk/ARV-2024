...
namespace Lexer
{
    Graph graphs[N_GRAPHS] =	// соответствие лексем и токенов
    {
        ...
        { LEX_LITERAL, FST::FST(GRAPH_SIXTEEN) },
    ...
    bool isOctLiteral(char* ch)
    {
        if (ch[0] == '1' && ch[1] == '6' && ch[2] == 'x')
            return true;
        return false;
    }

    // из 16 в 10
    char* for_10(char* st, Log::LOG& log, int curline)
    {
        char res[20];
        int decimalValue = 0;
        int base = 1;  // Значение 16^0

        for (int i = strlen(st) - 1; i > 2; --i) {
            if (st[i] >= '0' && st[i] <= '9') {
                decimalValue += (st[i] - '0') * base;
            }
            else if (st[i] >= 'A' && st[i] <= 'F') {
                decimalValue += (st[i] - 'A' + 10) * base;
            }
            else if (st[i] >= 'a' && st[i] <= 'f') {
                decimalValue += (st[i] - 'a' + 10) * base;
            }
    ...
    IT::STDFNC getStandFunction(char* id)
    {
        ...
        if (!strcmp(STRCMPP, id))
            return IT::STDFNC::F_STRCMPP;
        ...
        bool& lex_ok)			// флаг ошибки(по ссылке)
    {
        ...
        else	// если идентификатор
        {
            ...
            if (isFunc)
            {
                ...
                case IT::STDFNC::F_STRCMPP:
                {
                    itentry->idtype = IT::IDTYPE::S;
                    itentry->iddatatype = STRCMPP_TYPE;
                    itentry->value.params.count = STRCMPP_PARAMS_CNT;
                    itentry->value.params.types = new IT::IDDATATYPE[STRCMPP_PARAMS_CNT];
                    for (int k = 0; k < STRCMPP_PARAMS_CNT; k++)
                        itentry->value.params.types[k] = IT::STRCMPP_PARAMS[k];
                    break;
                }
    ...
    bool analyze(LEX& tables, In::IN& in, Log::LOG& log, Parm::PARM& parm)		// анализ действий при встрече соответствующих лексем
    {
        ...
        int curline;
        std::stack <char*> scopes;			// область видимости
                ...
                if (FST::execute(fst))
                {
                    ...
                    case LEX_LITERAL:
                    {
                        ...
                        if (isLiteral(curword))
                            if (!isOctLiteral(curword))
                                strcpy_s(id, curword);
                            else
                                strcpy_s(id, for_10(curword, log, curline));
                        ...
                        else     // повторный идентификатор
                        {
                            ...
                            if (i > 0 && tables.lextable.table[i - 1].lexema == LEX_NEW || tables.lextable.table[i].lexema == LEX_NEW
                                ...
                                || tables.lextable.table[i - 1].lexema == LEX_LITERAL_NUMO || tables.lextable.table[i - 1].lexema == LEX_NOT)
                            {
